defmodule AddressUS.Parser.Standardizer do
  ...
  def pre_standardize_address(messy_address, true) do
    ...
    |> safe_replace(~r/\b([A-Z])\.(?![A-Z]{3,})/, "\\1\\2")
    |> safe_replace(~r/(?!\d)\.(?!\d)/, " ")
  ...
  def handle_double_number_streets(addr) do
    if caps = Regex.run(~r/^(\d+) (\d+) ([A-Z].*)/, addr) do
      addr_num = Enum.at(caps, 1)
      poss_st_name = Enum.at(caps, 2)
      after_nums = Enum.at(caps, 3)
      ...
      res =
        case {List.first(split_after_nums), ordinal} do
          {"ST", "ST"} ->
            "#{addr_num} #{poss_st_name}ST #{rest}"

          {"ND", "ND"} ->
            "#{addr_num} #{poss_st_name}ND #{rest}"

          {"RD", "RD"} ->
            "#{addr_num} #{poss_st_name}RD #{rest}"

          {"TH", "TH"} ->
            "#{addr_num} #{poss_st_name}TH #{rest}"

          {"ST", ord} ->
            "#{addr_num} #{poss_st_name}#{ord} #{after_nums}"

          {ave, ord} when ave in ["AVE", "AVENUE"] ->
            # if ave_x is detected, it's too dangerous to clean up so we just pass the original addr
            if ave_x?, do: addr, else: "#{addr_num} #{poss_st_name}#{ord} #{after_nums}"
